...
bool world_sim_puede_moverse(state_t *state, short sustancia, int x, int y)
{
  ...
  {
  case SAND:
    if (state->board[x][y] == AIR)
    {
      return true;
    }
    else if (state->board[x][y] == SAND)
    {
      return false;
    }
    else if (state->board[x][y] == WATER && drand48() < 0.5)
    {
      return true;
    }
    else if (state->board[x][y] == ROCK)
    {
      return false;
    }
    else if (state->board[x][y] == OIL)
    {
      return true;
    }
    else if (state->board[x][y] == HUMO)
    {
      return true;
    }
  ...
  case WATER:
    ...
    else if (state->board[x][y] == OIL && drand48() < 0.5)
    {
      return true;
    }
    else if (state->board[x][y] == HUMO)
  ...
  case ROCK:
    ...
    else if (state->board[x][y] == SAND && drand48() < 0.2)
    {
      return true;
    }
    else if (state->board[x][y] == WATER && drand48() < 0.9)
    {
      return true;
    }
    else if (state->board[x][y] == ROCK)
    ...
    else if (state->board[x][y] == OIL && drand48() < 0.9)
    {
      return true;
    }
    else if (state->board[x][y] == HUMO)
...
{
  if (world_sim_puede_moverse(state, sustancia, x, y + 1))
  { // Mover abajo
...
bool sand_sim_mover_izq_der(state_t *state, short sustancia, bool seHaMovidoFlags[N][N], int x, int y)
{
  ...
  if (primeroIzquierda)
  {
    if (world_sim_puede_moverse(state, sustancia, x - 1, y))
    { // Mover a la izquierda
      sand_sim_mover(state, seHaMovidoFlags, x, y, x - 1, y);
      return true;
    }
    else if (world_sim_puede_moverse(state, sustancia, x + 1, y))
    { // Mover a la derecha
      sand_sim_mover(state, seHaMovidoFlags, x, y, x + 1, y);
      return true;
    }
...
bool sand_sim_mover_abajo_diagonal(state_t *state, short sustancia, bool seHaMovidoFlags[N][N], int x, int y)
{
  ...
  if (primeroIzquierda)
  {
    if (world_sim_puede_moverse(state, sustancia, x - 1, y + 1))
    { // Mover a la izquierda
      sand_sim_mover(state, seHaMovidoFlags, x, y, x - 1, y + 1);
      return true;
    }
    else if (world_sim_puede_moverse(state, sustancia, x + 1, y + 1))
    { // Mover a la derecha
      sand_sim_mover(state, seHaMovidoFlags, x, y, x + 1, y + 1);
      return true;
    }
...
void world_sand_sim(SDL_Renderer *renderer, state_t *state)
{
  ...
  if (state->mode == RUNNING_MODE)
  {
    ...
    for (int i = 0; i < MOVES_PER_FRAME; i++)
    {
      ...
      char str[128];
      sprintf(str, "void world_sand_sim function, # Of threads:%d , Thread: %d, FPS: %d , Time elapsed (s): %ld.%06ld", 8,
              iam,
              fps_sandsim,
